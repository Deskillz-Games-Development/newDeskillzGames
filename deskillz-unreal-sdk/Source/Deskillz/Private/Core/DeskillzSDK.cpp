...
void UDeskillzSDK::InitializeWithCredentials(const FString& InAPIKey, const FString& InGameId, EDeskillzEnvironment InEnvironment)
{
	...
	UE_LOG(LogDeskillz, Log, TEXT("Initializing Deskillz SDK v%s"), SDK_VERSION);
...
void UDeskillzSDK::GetTournaments()
{
	...
	FString Endpoint = FString::Printf(TEXT("/games/%s/tournaments"), *GameId);
	...
	MakeAPIRequest(Endpoint, TEXT("GET"), nullptr,
		[this](TSharedPtr<FJsonObject> Response, FDeskillzError Error)
		{
			...
			if (Error.IsError())
			{
				OnTournamentsReceived.Broadcast(Tournaments, Error);
				return;
			}
			
			// Parse tournament array
			const TArray<TSharedPtr<FJsonValue>>* TournamentArray;
			if (Response->TryGetArrayField(TEXT("tournaments"), TournamentArray))
			{
				for (const TSharedPtr<FJsonValue>& TournamentValue : *TournamentArray)
				{
					TSharedPtr<FJsonObject> TournamentObj = TournamentValue->AsObject();
					if (!TournamentObj.IsValid()) continue;
					...
					Tournament.GameId = TournamentObj->GetStringField(TEXT("gameId"));
					Tournament.PrizePool = TournamentObj->GetNumberField(TEXT("prizePool"));
					Tournament.MaxPlayers = TournamentObj->GetIntegerField(TEXT("maxPlayers"));
					Tournament.CurrentPlayers = TournamentObj->GetIntegerField(TEXT("currentPlayers"));
					Tournament.MatchDurationSeconds = TournamentObj->GetIntegerField(TEXT("matchDuration"));
					
					// Parse entry fee
					if (TSharedPtr<FJsonObject> EntryFeeObj = TournamentObj->GetObjectField(TEXT("entryFee")))
					{
						Tournament.EntryFee.Amount = EntryFeeObj->GetNumberField(TEXT("amount"));
						FString CurrencyStr = EntryFeeObj->GetStringField(TEXT("currency"));
						// Map currency string to enum
						if (CurrencyStr == TEXT("BTC")) Tournament.EntryFee.Currency = EDeskillzCurrency::BTC;
						else if (CurrencyStr == TEXT("ETH")) Tournament.EntryFee.Currency = EDeskillzCurrency::ETH;
						else if (CurrencyStr == TEXT("SOL")) Tournament.EntryFee.Currency = EDeskillzCurrency::SOL;
						else if (CurrencyStr == TEXT("USDT")) Tournament.EntryFee.Currency = EDeskillzCurrency::USDT;
						else if (CurrencyStr == TEXT("USDC")) Tournament.EntryFee.Currency = EDeskillzCurrency::USDC;
					}
					
					// Parse status
					FString StatusStr = TournamentObj->GetStringField(TEXT("status"));
					if (StatusStr == TEXT("open")) Tournament.Status = EDeskillzTournamentStatus::Open;
					else if (StatusStr == TEXT("inProgress")) Tournament.Status = EDeskillzTournamentStatus::InProgress;
					else if (StatusStr == TEXT("completed")) Tournament.Status = EDeskillzTournamentStatus::Completed;
					
					// Parse match type
					FString TypeStr = TournamentObj->GetStringField(TEXT("matchType"));
					Tournament.MatchType = (TypeStr == TEXT("synchronous")) 
...
void UDeskillzSDK::GetTournamentsFiltered(EDeskillzTournamentStatus Status, EDeskillzMatchType MatchType, float MinEntryFee, float MaxEntryFee)
{
	...
	switch (Status)
	{
		case EDeskillzTournamentStatus::Open: StatusFilter = TEXT("open"); break;
		case EDeskillzTournamentStatus::InProgress: StatusFilter = TEXT("inProgress"); break;
		case EDeskillzTournamentStatus::Completed: StatusFilter = TEXT("completed"); break;
		default: StatusFilter = TEXT(""); break;
	}
	
	FString TypeFilter = (MatchType == EDeskillzMatchType::Synchronous) ? TEXT("synchronous") : TEXT("asynchronous");
	
	FString Endpoint = FString::Printf(
		TEXT("/games/%s/tournaments?status=%s&type=%s&minFee=%.2f&maxFee=%.2f"),
		*GameId, *StatusFilter, *TypeFilter, MinEntryFee, MaxEntryFee);
	
	MakeAPIRequest(Endpoint, TEXT("GET"), nullptr,
		[this](TSharedPtr<FJsonObject> Response, FDeskillzError Error)
		{
			// Same parsing logic as GetTournaments
			GetTournaments();
		});
}
...
void UDeskillzSDK::GetTournamentDetails(const FString& TournamentId)
{
	...
	FString Endpoint = FString::Printf(TEXT("/tournaments/%s"), *TournamentId);
	...
	MakeAPIRequest(Endpoint, TEXT("GET"), nullptr,
		[this](TSharedPtr<FJsonObject> Response, FDeskillzError Error)
		{
			...
			// Tournament details fetched - can emit specific delegate if needed
			UE_LOG(LogDeskillz, Log, TEXT("Tournament details received"));
		});
}
...
void UDeskillzSDK::JoinTournament(const FString& TournamentId, EDeskillzCurrency Currency)
{
	...
	UE_LOG(LogDeskillz, Log, TEXT("Joining tournament: %s"), *TournamentId);
	
	TSharedPtr<FJsonObject> RequestBody = MakeShareable(new FJsonObject());
	RequestBody->SetStringField(TEXT("tournamentId"), TournamentId);
	RequestBody->SetStringField(TEXT("currency"), GetCurrencyString(Currency));
	
	MakeAPIRequest(TEXT("/tournaments/join"), TEXT("POST"), RequestBody,
		[this, TournamentId](TSharedPtr<FJsonObject> Response, FDeskillzError Error)
		{
			if (Error.IsError())
			{
				UE_LOG(LogDeskillz, Error, TEXT("Failed to join tournament: %s"), *Error.Message);
				BroadcastError(Error);
				return;
			}
			
			UE_LOG(LogDeskillz, Log, TEXT("Successfully joined tournament: %s"), *TournamentId);
			
			// Refresh wallet balances after entry fee
			GetWalletBalances();
		});
}
...
void UDeskillzSDK::LeaveTournament(const FString& TournamentId)
{
	...
	MakeAPIRequest(TEXT("/tournaments/leave"), TEXT("POST"), RequestBody, nullptr);
}
...
void UDeskillzSDK::StartMatchmaking(const FString& TournamentId)
{
	if (!IsReady() || !bIsAuthenticated)
	{
		BroadcastError(FDeskillzError(EDeskillzErrorCode::AuthenticationFailed, TEXT("Must be authenticated")));
		return;
	}
	
	if (bIsMatchmaking)
	{
		UE_LOG(LogDeskillz, Warning, TEXT("Already in matchmaking"));
		return;
	}
	
	UE_LOG(LogDeskillz, Log, TEXT("Starting matchmaking for tournament: %s"), *TournamentId);
	
	bIsMatchmaking = true;
	
	TSharedPtr<FJsonObject> RequestBody = MakeShareable(new FJsonObject());
	RequestBody->SetStringField(TEXT("tournamentId"), TournamentId);
	RequestBody->SetStringField(TEXT("playerId"), CurrentPlayer.PlayerId);
	
	MakeAPIRequest(TEXT("/matchmaking/join"), TEXT("POST"), RequestBody,
		[this, TournamentId](TSharedPtr<FJsonObject> Response, FDeskillzError Error)
		{
			if (Error.IsError())
			{
			...
			// Matchmaking started - WebSocket will receive match found event
			UE_LOG(LogDeskillz, Log, TEXT("Matchmaking started, waiting for opponent..."));
		});
}
...
void UDeskillzSDK::CancelMatchmaking()
{
	...
	UE_LOG(LogDeskillz, Log, TEXT("Cancelling matchmaking"));
	
	MakeAPIRequest(TEXT("/matchmaking/cancel"), TEXT("POST"), nullptr, nullptr);
	bIsMatchmaking = false;
}
